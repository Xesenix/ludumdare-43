...
function ReportContainerComponent({ app }) {
	const dictionary = Array.from((app as any)._bindingDictionary.getMap().entries()).sort(([a], [b]) => a > b ? 1 : -1);
	const phraseRef = React.useRef();
	const typeRef = React.useRef();
	const [ phrase = '', setPhrase ] = React.useState('');
	const [ type = '', setType ] = React.useState('');
	const search = React.useCallback(() => {
		setPhrase((phraseRef.current.value || '').toLowerCase());
		setType((typeRef.current.value || '').toLowerCase());
	}, [setPhrase, setType]);
